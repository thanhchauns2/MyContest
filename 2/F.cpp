...
bool check(ll n, ll U, ll R, ll D, ll L)
{
	if (U > n || R > n || D > n || L > n) return false;
	if (n < 2) return false;
	if ((U == 0 || D == 0) && (R == n || L == n)) return false;
	if ((U == n || D == n) && (R == 0 || L == 0)) return false;
	if ((U == 1 || D == 1) && (R == n && L == n)) return false;
	if ((U == n && D == n) && (R == 1 || L == 1)) return false;
	if ((U == 0 && D == 0) && (R == n-1 || L == n-1)) return false;
	if ((U == n-1 || D == n-1) && (R == 0 && L == 0)) return false;
	if ((U == n-1 && D == n || U == n && D == n-1) && (R == 1 && L == 1)) return false;
	if ((U == 1 && D == 1) && (R == n && L == n-1 || R == n-1 && L == n)) return false;
	if ((U == n-1 && D == n-1) && (R == 0 && L == 1 || R == 1 && L == 0)) return false;
	if ((U == 1 && D == 0 || U == 0 && D == 1) && (R == n-1 && L == n-1)) return false;
	return true;
}
...
int main()
{
	...
	Begin()
	{
		ll n,U,R,D,L;
		cin>>n>>U>>R>>D>>L;
		check(n,U,R,D,L) ?  cout<<"YES" : cout<<"NO";
		cout<<endl;
	}
}
